...
- **Мокирование переменных окружения**: Переменные `HOME` и `USERPROFILE` временно изменяются для тестов
- **Мокирование функций**: Функция `os.UserHomeDir()` заменяется на мок, который возвращает временную директорию теста
...
- Файлы создаются только во временных директориях
